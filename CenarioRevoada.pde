class CenarioRevoada extends Cenario {
  PVector posIni;
  PApplet p5;
  ...
  CenarioRevoada(PApplet _p5, float angX, float angY, float angZ, int radEsfera, String _nome){
  ...
  for(int i=0; i<particleNum; i++){
    particleCollection.add(new PasaroA(p5, random(0, p5.width), random(0, p5.height), random (-p5.width, p5.width), 
                           random(-2,2), random(-2,2), random(-2,2) ));
    ((PasaroA)particleCollection.get(particleCollection.size()-1)).ptarget = ctarget;
  }
}
...
//  p5.sphere(50);   
  ...
  for(int i=0; i<particleCollection.size(); i++){
    PasaroA myParticle = (PasaroA) particleCollection.get(i);
    myParticle.run();
  }
